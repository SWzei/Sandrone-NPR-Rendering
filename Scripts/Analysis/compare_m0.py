"""Quantitative M0 reference comparison (front + side, no registration warping)."""
...
def metrics(path: Path) -> tuple[dict, Image.Image]:
    ...
    hsv = np.array([colorsys.rgb_to_hsv(*pixel) for pixel in foreground[:: max(1, len(foreground) // 100_000)]])
    ...
    result = {
        ...
        "foreground_mean_hsv": [float(hsv[:, 0].mean()), float(hsv[:, 1].mean()), float(hsv[:, 2].mean())],
...
def main() -> None:
    if len(sys.argv) != 7:
        raise SystemExit("usage: compare_m0.py REF_FRONT RENDER_FRONT REF_SIDE RENDER_SIDE REPORT_JSON SHEET_PNG")
