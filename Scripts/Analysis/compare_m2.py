"""Quantitative M2 comparison against references and the accepted M1 baseline.

No registration or image warping is applied.  The report therefore separates
colour/luminance statistics from framing differences instead of presenting a
pixel error as a likeness score.
"""
...
def main() -> None:
    if len(sys.argv) != 9:
        raise SystemExit(
            "usage: compare_m2.py REF_FRONT M1_FRONT M2_FRONT REF_SIDE M1_SIDE M2_SIDE REPORT_JSON SHEET_PNG"
        )
    paths = [Path(value) for value in sys.argv[1:7]]
    report_path, sheet_path = Path(sys.argv[7]), Path(sys.argv[8])
    names = [
        "reference_front", "m1_front", "m2_front",
        "reference_side", "m1_side", "m2_side",
    ]
    ...
    for view in ("front", "side"):
        ...
        m1_delta = delta(measured[f"m1_{view}"], reference)
        m2_delta = delta(measured[f"m2_{view}"], reference)
        measured[f"delta_m1_to_reference_{view}"] = m1_delta
        measured[f"delta_m2_to_reference_{view}"] = m2_delta
        measured[f"m2_change_from_m1_{view}"] = delta(measured[f"m2_{view}"], measured[f"m1_{view}"])
        measured[f"absolute_luminance_error_change_{view}"] = (
            abs(m2_delta["foreground_luminance_mean"]) - abs(m1_delta["foreground_luminance_mean"])
        )
