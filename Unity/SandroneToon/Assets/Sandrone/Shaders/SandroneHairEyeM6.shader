    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                // Exact M4 prefix: required by the reused M4 ShadowCaster pass.
                float4 _BaseMap_ST;
                float4 _BaseColor;
                float _RampRow;
                float _RampRowCount;
                ...
                float4 _HeadRightWS;
                float4 _HeadUpWS;
                // M6-only fields are append-only.
                float _M6Role;
                float _EyeFlatLighting;
                float _HairSpecIntensity;
                float _HairSpecPower;
                float _HairSpecThreshold;
                float _HairSpecSoftness;
                float4 _HairSpecColor;
                float _M6HairSpecWeight;
                float _M6EyeLayerWeight;
                float _M6DebugMode;
                float _M6StencilRef;
                float _M6StencilReadMask;
                float _M6StencilWriteMask;
                float _M6StencilComp;
                float _M6StencilPass;
                float _M6AuditSlotId;
            CBUFFER_END
            ...
            half4 Frag(Varyings input, FRONT_FACE_TYPE frontFace:FRONT_FACE_SEMANTIC):SV_Target
            {
                ...
                half3 T = normalize(input.tangentWS) * faceSign;
                ...
                // This is bounded LDR eye lighting, not M8 HDR emission. Cast shadow remains visible.
                // The eye term is a monotonic fill: on an already-lit iris it must never
                // replace a brighter authored Ramp value with a darker "flat" target.
                half3 flatEyeLighting = max(conventionalLighting,
                    light.color * light.distanceAttenuation * lerp(0.72h, 1.0h, castStyled));
                ...
                half strand = sqrt(saturate(1.0h - dot(T, H) * dot(T, H)));
                half tangentLobe = pow(strand, max(_HairSpecPower * 0.25h, 1.0h));
                ...
                half hairSpec = hairBand * control.r * _HairSpecIntensity * _M6HairSpecWeight *
                    IsHairBase() * castStyled * light.distanceAttenuation;
                half3 outputColor = saturate(baseLit + hairSpec * _HairSpecColor.rgb);
                if (_FeatureGroup > 2.5h && _FeatureGroup < 3.5h)
                    outputColor = saturate(outputColor * _OverlayColorBoost);

                if (_M6DebugMode > 0.5h && _M6DebugMode < 1.5h) outputColor = control.rrr * IsHairBase();
                else if (_M6DebugMode > 1.5h && _M6DebugMode < 2.5h) outputColor = tangentLobe.xxx * IsHairBase();
                else if (_M6DebugMode > 2.5h && _M6DebugMode < 3.5h) outputColor = hairSpec.xxx;
                else if (_M6DebugMode > 3.5h && _M6DebugMode < 4.5h)
                {
                    outputColor = _M6Role < 1.5h ? half3(0.7h,0.2h,0.2h) :
                        _M6Role < 2.5h ? half3(0.2h,0.8h,1.0h) :
                        _M6Role < 3.5h ? half3(0.25h,0.4h,1.0h) :
                        _M6Role < 4.5h ? half3(0.85h,0.65h,0.25h) : half3(1.0h,0.3h,0.75h);
                }
                else if (_M6DebugMode > 4.5h && _M6DebugMode < 5.5h) outputColor = baseSample.aaa * IsEyeRole();
                else if (_M6DebugMode > 5.5h && _M6DebugMode < 6.5h) outputColor = castStyled.xxx;
                else if (_M6DebugMode > 6.5h) outputColor = half3(_M6StencilRef / 3.0h, _M6StencilReadMask, _M6StencilWriteMask);
